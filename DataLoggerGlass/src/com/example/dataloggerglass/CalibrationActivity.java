...
public class CalibrationActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View view = this.getLayoutInflater().inflate(R.layout.activity_calibration, null);
		...
		mThread = new Thread() {
			...
			public void run() {
				...
				for (int i = 0; i < irValues.size() - 4; i++) {
					...
					Float left = (irValues.get(i - 3) + irValues.get(i - 2) + irValues.get(i - 1)) / 3.0f;
					Float right = (irValues.get(i + 1) + irValues.get(i + 2) + irValues.get(i + 3)) / 3.0f;
					...
					dValues.add((float) Math.pow(Math.pow(peak - (left + right) / 2.0f, 2), 0.5));
				}
				...
				for (float threshold = 3.0f; threshold < 7.0f; threshold += 0.1) {
					...
					Long lastBlinkTimestamp = 0l;
					if (timestampValues.size() > 0) {
						lastBlinkTimestamp = timestampValues.get(0);
					}
					for (int i = 0; i < dValues.size(); i++) {
						if (dValues.get(i) > threshold) {
							Long blinkTimestamp = timestampValues.get(i);
							if (blinkTimestamp > lastBlinkTimestamp + 300) {
								if (labelValues.get(i / 90) - 100 < timestampValues.get(i)
										&& timestampValues.get(i) < labelValues.get(i / 90) + 100) {
					...
					long endTime = 0l;
					if (timestampValues.size() > 0) {
						endTime = timestampValues.get(90);
					...
					Log.v("Monitoring Activity", "ir-progress threshold:" + threshold + "tp:" + truePositive + " fp:"
							+ falsePositive + " fn:" + falseNegative + " tn:" + trueNegative);
				}
				...
				edit.commit();
				Log.v("Monitoring Activity", "ir-finished threshold:" + threshold + "fMeasure:" + maxFmeasure);
				mThread.interrupt();
				finish();
			}
		};
